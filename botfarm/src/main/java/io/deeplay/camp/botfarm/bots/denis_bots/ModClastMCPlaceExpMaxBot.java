...
public class ModClastMCPlaceExpMaxBot extends Bot {
    ...
    public double maximumPlaceAlg(GameState root, int depth) {
        if(depth == 0){
            ...
            if(tacticUtility.getCurrentPlayerType() == PlayerType.FIRST_PLAYER){
                if(root.getArmyFirst().hasGeneral()){
                    generalMod = (root.getArmyFirst().getGeneralType() == currentGeneral) ? 5 : 1;
                }
            }
            else{
                if(root.getArmySecond().hasGeneral()){
                    generalMod = (root.getArmySecond().getGeneralType() == currentGeneral) ? 5 : 1;
                }
            }
        ...
        if(placeRoot.isEmpty()){
            ...
            return getMaxPlaceFromTasks(results).value;
        }
    }
