...
public class ClastPlaceExpMaxBot extends Bot {
            ...
            for (UtilityMoveResult moveBestClast : bestMoves) {
                RecursiveTask<UtilityMoveResult> task = new RecursiveTask<>() {
                    ...
                    protected UtilityMoveResult compute() {
                        double result = expectMaxAlg(gameState, moveBestClast.event, originDepth, true);
    ...
    public double alphaBetaMinMaxAlg(GameState root, int depth,  boolean maxPlayer) throws GameException {
        if(depth == 0 || root.getGameStage() == GameStage.ENDED){
        ...
        if(movesRoot.isEmpty()){
            if(root.getGameStage() == GameStage.ENDED){
                ...
                return alphaBetaMinMaxAlg(gameStateNode, depth, !maxPlayer);
            }
        } else {
            for (UtilityMoveResult moveEvent : bestMoves) {
                double v = expectMaxAlg(root, moveEvent.event, depth, maxPlayer);
    ...
    private double expectMaxAlg(GameState root, MakeMoveEvent event, int depth, boolean maxPlayer) throws GameException {
        ...
        for (StateChance chance : chancesRoot) {
            ...
            double v = alphaBetaMinMaxAlg(nodeGameState, depth-1, maxPlayer);
        ...
        for (UtilityMoveResult point : cluster.points) {
            RecursiveTask<UtilityMoveResult> task = new RecursiveTask<>() {
                ...
                protected UtilityMoveResult compute() {
                    ...
                    double result = expectMaxAlg(gameState, move, 1, maxPlayer);
